...
const getElementsFromTokens = (element, tokens) => {
  ...
  tokens.forEach((token => {
    try {
      switch (token) {
       case "combined":
        if (Deprecate$1.enabled) console.warn("In the next version of StimulusReflex, the 'combined' option to data-reflex-dataset will become 'ancestors'.");
...
const initialize = (application, {controller: controller, consumer: consumer, debug: debug, params: params, isolate: isolate, deprecate: deprecate} = {}) => {
  ...
  document.addEventListener("DOMContentLoaded", (() => {
    ...
    if (Deprecate$1.enabled && consumer) console.warn("Deprecation warning: the next version of StimulusReflex will obtain a reference to consumer via the Stimulus application object.\nPlease add 'application.consumer = consumer' to your index.js after your Stimulus application has been established, and remove the consumer key from your StimulusReflex initialize() options object.");
    if (Deprecate$1.enabled && IsolationMode.disabled) console.warn("Deprecation warning: the next version of StimulusReflex will standardize isolation mode, and the isolate option will be removed.\nPlease update your applications to assume that every tab will be isolated.");
  }), {
    once: true
  });
  ...
  Debug$1.set(!!debug);
  if (typeof deprecate !== "undefined") Deprecate$1.set(deprecate);
  ...
  Object.assign(controller, {
    ...
    stimulate() {
      ...
      setTimeout((() => {
        ...
        if (Deprecate$1.enabled && options["serializeForm"] === undefined && form) console.warn(`Deprecation warning: the next version of StimulusReflex will not serialize forms by default.\nPlease set ${Schema.reflexSerializeForm}="true" on your Reflex Controller Element or pass { serializeForm: true } as an option to stimulate.`);
...
export { index as default, initialize, register, useReflex };
